...
//prantik
  ...
  case OMPD_target_teams_distribute_simd: {
   ...
   // prantik multiple changes within
    while (Tok.isNot(tok::annot_pragma_openmp_end)) {
     // prantik change below:  Tok.isAnnotation()
     //  965               ? OMPC_unknown
     //   966               : FlushHasClause ? OMPC_flush
     //    967                                : getOpenMPClauseKind(PP.getSpelling(Tok));
     //
      OpenMPClauseKind CKind =
          Tok.isAnnotation()
              ? OMPC_unknown: IncHasClause ? OMPC_inc
